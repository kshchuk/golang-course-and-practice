...
public class MatrixMult {
    ...
    public static void parallelMatrixMultiply(Matrix a, Matrix b, Matrix c,
        ...
        // BroadCast sends array of info, takes offset, num of elements, Root Rank,
        // Communicator as args
        ...
        if (myRank == 0) {
            ...
            for (int i = 1; i < size; i++) {
                ...
                // EdgeCase if Rank End row -> (i + 1) * chunkSize is greater than number of
                // Actual Rows
                if (rankEndRow > rows)
                    rankEndRow = rows;
                ...
                // Buffer to Receive, RowOffset, num of Elements, Rank of Sender, Tag for
                // Message, Comm
            }
            mpi.MPI_Waitall(requests);
            // like async-await, wait for all other ranks to return result (Async -> reason
            // for Speedup)

        } else {
            // Other Ranks send their Results to rook rank
            mpi.MPI_Send(c.getValues(), start * c.getNCols(), (end - start) * c.getNCols(), 0, myRank,
                    mpi.MPI_COMM_WORLD);
